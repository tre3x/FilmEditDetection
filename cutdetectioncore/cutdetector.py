...
class predict():
    ...
    INPUT : conf, model_path, vid_path
    ...
    def get_hardcuts(self, vid_path):
        ...
        '''
        hardcuts, candidates, fps = hardcut.get_result(vid_path, self.config)
    ...
    def verify_cuts(self, vid_path, frames, ifSoftcut=False):
        ...
        '''
        if ifSoftcut:
            _, softcuts = softcut.softcut(self.model_path, self.config).getResult(vid_path, frames)
            return softcuts
        else:
            hardcut, _ = softcut.softcut(self.model_path, self.config).getResult(vid_path, frames)
            return hardcut
